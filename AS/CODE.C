...
extern int   autozp_opt;	/* auto zeropage mode for NES ONLY */
int check_preindexed(int idx);
...
getoperand(int *ip, int flag, int last_char)
{
	...
	default:
		...
		case '(':
			...
			if ( autozp_opt == 1 )
			{
				flag_preind = check_preindexed(*ip);
				if ( flag_preind < 0 )
					return (0);
				if ( flag_preind == 1 ) {
					mode = ZP_IND_Y | ABS_Y;
				...
				if ( flag_preind > 0 ) {
					...
					mode = ZP_X | ZP_Y | ABS_X | ABS_Y | ABS | ZP | ZP_IND_Y;
				}
			} else {
				/* same default */
		...
		if (code == 0x000000) {
			...
			if ( ( autozp_opt == 1 ) ) {
				//printf("**");
				if ( opval != 0x14 && opval != 0x40 && ( value & 0xFFFFFF00 ) == 0 ) {
					//printf("**");
					mode &= ( ZP | IMM );
				} else {
					//printf("*");
		...
		/* indexed modes */
			...
			// 2011/01/01 minachun add
			if ( ( flag_preind == 1 ) && ( value & 0xFFFFFF00 ) == 0 ) {
				mode &= ZP_IND_Y;
			} else {
...
//			if ( ( autozp_opt == 1 ) && (pass == LAST_PASS) ) {
...
#endif
		// 2011/01/10 minachun fixed.
		if ( autozp_opt == 1 && pass == LAST_PASS && opval != 0x14 && opval != 0x40 && ( mode & IMM ) == 0 && ( value & 0xFFFFFF00 ) == 0x00 )
		{
			mode &= ZP | ZP_X | ZP_Y | ZP_IND | ZP_IND_X | ZP_IND_Y;
		}
		// 2010/12/30 minachun end
	...
	mode &= flag;
	if (!mode) {
		error("Incorrect addressing mode! ww");
		return (0);
	}
...
check_preindexed(int idx)
{
	...
	//printf("idx_x=%d idx_y=%d idx_comma=%d end=%d\n",idx_x,idx_y,idx_comma,end);
